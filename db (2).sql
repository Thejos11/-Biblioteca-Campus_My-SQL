...
-- ---------------------------------------------------------------------
-- Triggers: actualización automática de disponibilidad en libros
-- ---------------------------------------------------------------------
-- Al registrar un nuevo préstamo (estado inicial 'prestado' o 'atrasado'),
-- se marca el libro asociado como NO disponible.
DELIMITER $$
...
FOR EACH ROW
BEGIN
    IF NEW.estado IN ('prestado', 'atrasado') THEN
        UPDATE libros l
        JOIN publicaciones p ON p.id_libro = l.id_libro
        SET l.disponibilidad = FALSE
        WHERE p.id_publicacion = NEW.id_publicacion;
    END IF;
END$$

-- Al actualizar un préstamo:
--   - si pasa a 'devuelto', el libro vuelve a estar disponible.
--   - si pasa a 'prestado' o 'atrasado', el libro se marca como no disponible.
...
BEGIN
    IF NEW.estado = 'devuelto' AND OLD.estado <> 'devuelto' THEN
        UPDATE libros l
        JOIN publicaciones p ON p.id_libro = l.id_libro
        SET l.disponibilidad = TRUE
        WHERE p.id_publicacion = NEW.id_publicacion;
    ELSEIF NEW.estado IN ('prestado', 'atrasado') AND OLD.estado <> NEW.estado THEN
        UPDATE libros l
        JOIN publicaciones p ON p.id_libro = l.id_libro
        SET l.disponibilidad = FALSE
        WHERE p.id_publicacion = NEW.id_publicacion;
